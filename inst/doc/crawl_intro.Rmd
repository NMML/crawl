...
Some initial tidying of the data has been done prior to inclusion with the package [dplyr](https://cran.rstudio.com/package=dplyr) was used for this process and this resulted in a `tbl_df` object. 
...
Note that while the `date_time` columns are duplicated, values for `quality`, `latitude`, and `longitude` differ. Since we have no objective method for selecting one record over the other, we will opt to adjust one of the duplicate times by increasing the `date_time` by 1 second. In this example, we use the `make.time.unique()` function from the [xts](https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html) package (along with some dplyr and [tidyr](https://cran.rstudio.com/web/packages/tidyr/vignettes/tidy-data.html) magic) to create a new column, `unique_posix`.
...
Also notice that we emply the `parallel::mclapply()` function in order to perform this procedure in parallel and save some time.
